...
async def imgSearcher(bot: Bot, event: MessageEvent):
    global dataGet
    if ("搜图" in str(event.get_plaintext()) or int(event.senderUin) in dataGet):
        pat=""
        ...
        try:
            result, piccc = await saucenoS(url=img_url, proxies=proxy)
            ...
            dataGet.get(int(event.senderUin)).append("sauceno搜图失败，无结果或访问次数过多，请稍后再试")
