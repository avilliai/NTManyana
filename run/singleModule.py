...
picGetter=on_keyword("图", "壁纸","pic")
picGetter2= on_keyword("张")#on_regex(r'(\d+)张(\w+)',flags=re.IGNORECASE)
hisToday=on_keyword("今")
...
async def make_jokes(bot: Bot, event: MessageEvent):
    ...
    x = str(event.get_plaintext()).strip()[1:-2]
    joke = get_joke(x)
...
async def jiangzhuang(bot: Bot, event: MessageEvent):
    if str(event.get_plaintext()).startswith("/奖状"):
        try:
