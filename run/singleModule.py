...
logger1.info("抽卡/运势模块启动")
logger1.info("额外的功能 启动完成")
...
async def meme(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            ...
            logger1.info(r.json().get("preview")[-1])
...
async def handle_group_message(bot: Bot, event: MessageEvent):
    ...
    if get_number(str(event.get_plaintext())) == None:
        ...
        logger1.info("图片获取指令....数量：" + str(picNum))
...
async def setuHelper(bot: Bot, event: MessageEvent):
    ...
    if match1:
        logger1.info("提取图片关键字。 数量: " + str(match1.group(1)) + " 关键字: " + match1.group(2))
        data = {"tag": ""}
        ...
        for i in range(a):
            try:
                ...
                logger1.error("涩图请求出错")
                await bot.send(event, "请求出错，请稍后再试")
                return
            logger1.info("发送图片: " + path)
            try:
                await bot.send_group_message(event.scene, MessageSegment.image(Path(path)))
                logger1.info("图片发送成功")
            except:
                logger1.error("图片发送失败")
...
async def historyToday(bot: Bot, event: MessageEvent):
    ...
    if match:
        ...
        logger1.info("获取历史上的今天")
        logger1.info(str(dataBack))
...
async def moyuToday(bot: Bot, event: MessageEvent):
    ...
    logger1.info("获取摸鱼人日历")
    path = await moyu()
    logger1.info("成功获取到摸鱼人日历")
    await bot.send_group_message(event.scene, MessageSegment.image(Path(path)))
...
async def NasaHelper(bot: Bot, event: MessageEvent):
    ...
    if datetime.datetime.now().strftime('%Y-%m-%d') in data.keys():
        ...
        try:
            ...
            logger1.info("获取到结果" + str(response.json()))
            # logger1.info("下载缩略图")
...
async def NasaHelper(bot: Bot, event: MessageEvent):
    if str(event.get_plaintext()).startswith("/sd"):
        try:
            ...
            return
        logger1.info("拆分获取prompt:" + str(ls))
        try:
...
async def moyuToday(bot: Bot, event: MessageEvent):
    ...
    logger1.info("获取方舟抽卡结果")
    try:
        path = await arkGacha()
        logger1.info("成功获取到抽卡结果")
        await bot.send_group_message(event.scene, MessageSegment.image(Path(path)))
    except:
        logger1.error("皱皮衮")
...
async def moyuToday(bot: Bot, event: MessageEvent):
    ...
    logger1.info("获取星铁抽卡结果")
    try:
        path = await starRailGacha()
        logger1.info("成功获取到星铁抽卡结果")
        await bot.send_group_message(event.scene, MessageSegment.image(Path(path)))
    except:
        logger1.error("穹批衮")
        await bot.send(event, "获取抽卡结果失败，请稍后再试")
